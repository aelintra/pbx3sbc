...
if (!$_SESSION['read_only']) {
	...
	if ($action=="edit") {
		...
		$sql='SELECT domain, setid '.($has_attrs?", attrs":""). ' FROM '.$table.' where id=?';
	...
	<?php csrfguard_generate(); ?>
		...
		<tr>
			<td colspan="2" align="center">
			    <input type="submit" name="add" value="<?=$button?>" class="formButton">&nbsp;&nbsp;&nbsp;
			    <?php if ($action=="edit") {?>
	...
	(function() {
	  ...
	  if (domainField) {
	    ...
	    // Trigger validation on load for edit forms
	    if (domainField.value) {
	      validate_input("domain", "domain_ok", "^(([0-9]{1,3}\\.){3}[0-9]{1,3})|(([A-Za-z0-9-]+\\.)+[a-zA-Z]+)$", null, "");
	    }
	  }
	  var setidField = document.getElementById("setid");
	  if (setidField) {
	    setidField.addEventListener("input", function() {
	      validate_input("setid", "setid_ok", "^[0-9]+$", null, "");
	    });
	    // Trigger validation on load for edit forms
	    if (setidField.value) {
	      validate_input("setid", "setid_ok", "^[0-9]+$", null, "");
