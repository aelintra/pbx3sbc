...
**Control Panel Version:** 9.x (from GitHub)  
...
$config->db_pass = "rigmarole";  // Must match MySQL user password
...
**Note:** The grey/green button behavior is actually a feature - buttons are disabled (grey) when form is invalid and enabled (green) when valid. This is controlled by JavaScript validation in `/var/www/opensips-cp/web/common/forms.php`.

### Issue 2: Edit Form Blank Screen
...
**Solution:** Updated `/var/www/opensips-cp/config/db.inc.php` with correct password (`rigmarole`)
...
- **Database:** MySQL database `opensips` (user: `opensips`, password: `rigmarole`)
...
```bash
mysql opensips -u opensips -p'rigmarole' -e "SELECT COUNT(*) FROM domain;"
```
...
1. **Control panel shows database error:**
   ...
   - Verify MySQL user has access: `mysql opensips -u opensips -p'rigmarole'`
